...
import { getAllMovements } from '../../repository/donationsApi/movements'
...
export default function Movements() {
	...
	const [currentConference, setCurrentConference] = useState<number>(0)
	...
	const getMovements = async () => {
		try {
			setIsLoading(true)
			const { data, totalPages: pTotalPages } = await getAllMovements({
	...
	const getData = async () => {
		try {
			const dataConferences = await getConferences()
			setAllConferences(dataConferences.data)
	...
	useEffect(() => {
		getMovements()
	}, [currentPage, dataLimit, currentConference])
		...
		<>
			...
			<Box w="100%" minHeight="100vh" bg="#E5E5E5">
				...
				>
					...
					<Box w="100%" padding="20px">
						...
						<Select
							...
							h="56px"
							mt="4px"
							placeholder={`Selecione uma conferencia`}
							onChange={({ target: { value } }) => {
								setCurrentConference(parseInt(value))
							}}
						>
							{allConferences?.map((conference) => {
								return (
									<option
										key={`${conference.id}-${conference.description}`}
										value={conference.id}
									>
					...
					<Box>
						<Text fontWeight="bold" fontSize="20px" margin="0px 20px">
              ...
              console.log("TCL: item", item)
								return {
									name: item.movement_description,
